...
1. **启动管理器** —— 双击 `启动 Kidai Plugin Remote.cmd`（浏览器独立窗口），
   或用 Client 版 exe（原生窗口）
2. **查看插件** —— 「已安装插件」页：原生/第三方徽标、启停状态、版本、路径、
   安装时间；支持排序 / 筛选 / 宽窄视图
3. **启停插件** —— 行内开关切换：写入 `$DSH_HOME/cordis.patch.yml`（与市场
   同一套写入），重启 DSH 生效；DSH 运行中 HMR 即时生效
4. **启动 DSH** —— 右侧「▶ 启动 DSH Desktop」；建议先「🔍 启动前预检分析」
   提前发现高危问题（未解析 bundle、补丁损坏、重复条目等）
5. **隔离运行** —— DSH 起不来时用「🧊 隔离运行」：本次启动禁用全部第三方插件，
   跑纯原生基线分析；退出自动恢复原配置
6. **快照与回滚** —— 每次启动/变更前自动留快照；出问题到「快照与回滚」页，
   选「已验证」快照一键回滚（配置 + 第三方插件目录），重启 DSH 即回到变更前
7. **故障报告** —— 启动失败自动弹出诊断报告：退出码、日志线索、冲突分析、
   每条附修复建议；也可「复制报告」保存
