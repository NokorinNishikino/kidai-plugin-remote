...
import { resolveProfileDir, readDesktopBundlePatchText, readAsarText } from "./dsh-env.js";
...
export function readPackageManifest(dir) {
  try {
    // Strip a UTF-8 BOM if present — JSON.parse rejects it (a BOM sneaks in
    // when a package.json is rewritten by PowerShell -Encoding UTF8).
    const parsed = JSON.parse(readFileSync(join(dir, "package.json"), "utf8").replace(/^\uFEFF/, ""));
...
/** Absolute path of the shipped app archive (holds the bundled core layers). */
export function appArchivePath(env) {
  // The install anchor is `<install>\resources\app.asar.unpacked\package.json`,
  // so the archive is its grandparent's sibling — never an `.asar.unpacked`
  // entry of its own.
  const anchor = env?.installAnchor;
  if (typeof anchor === "string" && anchor.endsWith("package.json")) return join(dirname(dirname(anchor)), "app.asar");
  const install = env?.install;
  if (typeof install === "string" && install.length > 0) return join(install, "resources", "app.asar");
  return undefined;
}

/**
 * A bundle layer packaged INSIDE `app.asar`.
 *
 * DSH Desktop 2.x ships the Harness core (`@deepseek-ai/dsh-base`,
 * `@deepseek-ai/dsh-web-app`, …) inside the archive, where a plain Node
 * filesystem cannot see it. Reporting those as "dependency not installed" is a
 * false alarm that made the preflight claim DSH could not boot; the archive
 * entry is the evidence instead.
 */
function loadArchivedBundleLayer(env, packageName, packageDir, disabledBundles) {
  const asarPath = appArchivePath(env);
  if (asarPath === undefined || !existsSync(asarPath)) return undefined;
  const prefix = `node_modules/${packageName}`;
  const manifestText = readAsarText(asarPath, `${prefix}/package.json`);
  ...
  const declaredClean = declared === undefined ? undefined : declared.replace(/^\.\//, "");
  const patchPath = declaredClean === undefined ? undefined : `${asarPath}\u0000${prefix}/${declaredClean}`;
  let patches = [];
  if (declared !== undefined) {
    const patchText = readAsarText(asarPath, `${prefix}/${declared}`);
    if (patchText !== undefined) {
      patches = parsePatchText(patchText, patchPath, "bundle overlay");
    }
  }
  return {
    packageName,
    packageDir: `${asarPath}\u0000${prefix}`,
    manifest,
    patchPath,
    patches,
    mutable: isMutableBundle(packageName),
    disabled: disabledBundles.has(packageName),
    archived: true,
  };
}
...
export function loadProfileLayers(env, profileName, { skipDisabledBundles = true } = {}) {
  ...
  for (const packageName of bundleNames) {
    ...
    try {
      ...
      // The core layers live inside `app.asar` on Desktop 2.x: fall back to the
      // archive before reporting an unresolvable dependency.
      let archived;
      try {
        archived = loadArchivedBundleLayer(env, packageName, env.installAnchor, disabledBundles);
      } catch (archivedError) {
        layerProblems.push({
          packageName,
          message: archivedError instanceof Error ? archivedError.message : String(archivedError),
        });
        continue;
      }
      if (archived !== undefined) {
        layers.push(archived);
        continue;
      }
      layerProblems.push({
        packageName,
...
export function desktopOverlayPatches(env) {
  const patches = [];
  // Desktop 2.x keeps this patch inside `app.asar`; read it from the archive when
  // the unpacked copy is not on disk, so the desktop layer is not silently
  // dropped from the composition (which made the desktop-shell rows invisible).
  const layer = readDesktopBundlePatchText(env);
  if (layer !== undefined) {
    patches.push(...parsePatchText(layer.text, layer.source, "desktop overlay"));
  }
  return patches;
}
