...
export function listSnapshots(env, profileName) {
  ...
  try {
    ...
    for (const entry of readdirSync(root, { withFileTypes: true })) {
      ...
      let size = Number.isFinite(meta.size) ? meta.size : 0;
...
/** Whether a snapshot is a viable rollback target. A snapshot that was itself
 *  already used as a rollback target (status `rolled-back`) stays valid — the
 *  recorded state is intact, so it can be rolled back to again and again. */
export function rollbackCandidates(list) {
  return list.filter((snap) => snap.status === "verified" || snap.status === "pending" || snap.status === "failed" || snap.status === "rolled-back");
}
