...
export async function importZipAndRestore(env, profileName, composed, zipBuffer, { disableSuspected = false } = {}) {
  ...
  const restoreEntry = (zipRel, target, label) => {
    ...
    try {
      backupFile(env.dshHome, target, "import");
  ...
  for (const entry of entries) {
    ...
    const target = join(profileDir, "node_modules", ...name.split("/"));
    try {
      backupDirectory(env.dshHome, target, "import");
      mkdirSync(target, { recursive: true });
      writeFileSync(join(target, ...relPath.split("/")), entry.getData());
...
export function importFullEnvironmentZip(env, home, zipPath, { userData, profileName } = {}) {
  ...
  const restoreEntry = (entry) => {
    ...
    let target;
    if (entry.entryName.startsWith("dsh/")) target = join(home, entry.entryName.slice("dsh/".length));
    else if (entry.entryName.startsWith("desktop/")) target = join(userDataDir, entry.entryName.slice("desktop/".length));
    else return;
  ...
  if (linksEntry !== undefined) {
    try {
      ...
      for (const link of Array.isArray(links) ? links : []) {
        ...
        const target = link.path.startsWith("dsh/") ? join(home, link.path.slice("dsh/".length)) : join(home, link.path);
        try {
          mkdirSync(dirname(target), { recursive: true });
