...
import { resolveProfileDir, desktopExePath, resolveDshCliCommand } from "./dsh-env.js";
...
export async function runCliDump(env, profileName, overlayPath) {
  const command = resolveDshCliCommand(env?.install);
  if (command === undefined) {
    return {
      ok: false,
      exitCode: -1,
      stdout: "",
      stderr: "dsh CLI entry not found (DSH Desktop install missing or carries no packaged CLI)",
      composed: undefined,
      nodeInfo: undefined,
    };
  }
  ...
  const result = await runCli(command, args, env);
...
export function runCli(command, args, env, { timeoutMs = 45_000 } = {}) {
  return new Promise((resolve) => {
    ...
    // The Desktop 2.x bootstrap is exposed through the app's internal module
    // hooks, exactly as the app's own `bin\dsh.cmd` shim invokes it.
    const preArgs = command.kind === "desktop-bootstrap" ? ["--expose-internals"] : [];
    try {
      child = spawn(node.run, [...preArgs, ...command.args, ...args], {
        cwd: env.dshHome,
        env: spawnEnv,
        windowsHide: true,
        stdio: ["ignore", "pipe", "pipe"],
