...
/** Whether the process is an Electron runtime (main or ELECTRON_RUN_AS_NODE). */
export function isElectronRuntime() {
  return typeof process.versions?.electron === "string";
}

/** Cache of asar entry reads (the archive is immutable for one install). */
const asarTextCache = new Map();

/**
 * Extract one asar entry.
 *
 * Electron's `node:fs` hands back the VIRTUAL archive (an empty view) for an
 * `app.asar` path, so the raw bytes are read with asar support temporarily
 * disabled (`process.noAsar`); a plain Node runtime needs no such dance and
 * reads the archive as the ordinary file it is.
 */
function readAsarEntryText(asarPath, relPath) {
  // Normalize the entry path: manifests commonly declare `./cordis.patch.yml`,
  // and a literal `.` segment is not an archive key.
  ...
  const read = () => {
    ...
    try {
      ...
      // asar nests directories as `{ files: {...} }`, so every segment descends
      // through `.files` (`files.node_modules.files["@scope"].files[name]`).
      let entry = header;
  ...
  if (!isElectronRuntime()) return read();
...
export function readAsarText(asarPath, relPath) {
  const key = `${asarPath}\u0000${relPath}`;
  if (asarTextCache.has(key)) return asarTextCache.get(key);
  const text = readAsarEntryText(asarPath, relPath);
  asarTextCache.set(key, text);
  return text;
}
